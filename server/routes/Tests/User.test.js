...
beforeAll(async () => {
  ...
  const new_user=new users({username:"user",type:"User",email:"user@gmail.com",phone_number:"0985674328",password:"User12,"});
  await new_user.save();
  app=express();
...
describe("INTEGRATION TEST - User Route",()=>{
    ...
    it("User Is Able To Successfully Change Password",async()=>{
      const res=await request(app).put("/users/checkpassword").send({username:"user",current_password:"User12,",new_password:"Newpassword12,"});
      expect(res.status).toBe(400);
    })


    it("Successfully Adds New Users To Database",async()=>{
      const res=await request(app).post("/users/signup").send({username:"jacob",email:"george@outlook.com",phone_number:"5438922345",password:"George123,"});
      expect(res.status).toBe(200);
    });
});
