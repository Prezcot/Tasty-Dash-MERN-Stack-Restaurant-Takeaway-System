...
test("The refund data endpoint is ready", async () => {
  const response = await request(app).get(
    "/admin_dashboard_data/receive/refund_data"
  );
  expect(response.status).toBe(200);
});
test("The order collected data endpoint is ready", async () => {
  const response = await request(app).get(
    "/admin_dashboard_data/receive/order_collected_data"
  );
  expect(response.status).toBe(200);
...
test("The set order status endpoint is ready", async () => {
  const response = await request(app)
    .put("/admin_dashboard_data/set_order_status")
    .send({ object_id: id, order_status: "testStatus" });
  expect(response.status).toBe(200);
});
...
test("The move to refund endpoint is ready", async () => {
  const response = await request(app)
    .post("/admin_dashboard_data/move_to_refund")
    .send({ object_id: id });
  expect(response.status).toBe(200);
});
test("The move to collected orders endpoint is ready", async () => {
  const response = await request(app)
    .post("/admin_dashboard_data/move_to_collected_orders")
    .send({ object_id: id });
  expect(response.status).toBe(200);
});
