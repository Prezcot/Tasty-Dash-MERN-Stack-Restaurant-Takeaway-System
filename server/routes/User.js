...
router.get("/authentication", async (req, res, next) => {
  //This route handler handles all security requests
...
router.post("/signin", async (req, res, next) => {
  //This route handler handles all signin requests
  ...
  try {
    ...
    if (query.length > 0 && valid) {
      if (query[0].type == "User") {
        ...
        res
          .status(200)
          .json({ message: "Account Registered", user: "Admin", token: token });
      }
    } else {
      res
        .status(400)
        .json({ message: "Account Not Registered/Invalid Credentials" });
    }
  } catch (err) {
    res.status(400).json({ message: err });
  }
...
router.post("/signup", async (req, res, next) => {
  //This route handler handles all signup requests
  ...
  try {
    ...
    var find_phone_number = await users
      .find({ phonenumber: phone_number })
      .catch((err) => console.log(err));
    var find_email = await users
      .find({ email: email })
      .catch((err) => console.log(err));
    if (find_user.length > 0) {
      ...
      await new_user
        .save()
        .then(() =>
          res.status(200).json({ message: "Successful Register", token: token })
        )
        .catch((err) => res.status(400).json({ message: err }));
