...
function Menu() {
  ...
  const [quantity_map, setQuantityMap] = useState(
    JSON.parse(sessionStorage.getItem("menu_cart"))
  );
  ...
  const updateItems = (itemName, itemImage, action) => {
    setQuantityMap((prev_map) => {
      ...
      const new_quantity =
        action === "add"
          ? current_quantity + 1
          : Math.max(current_quantity - 1, 0);

      const updated_map =
        new_quantity === 0
          ? Object.fromEntries(
              Object.entries(prev_map).filter(([key]) => key !== itemName)
            )
          ...
          {items
            ...
            .map((item) => (
              <Item
                ...
                onAddToCart={() =>
                  updateItems(item.itemName, item.itemImage, "add")
                }
                onRemoveFromCart={() =>
                  updateItems(item.itemName, item.itemImage, "remove")
                }
              />
            ))}
        </div>
