...
describe("INTEGRATION TEST - LIVE ORDERS COMPONENT", () => {
  ...
  it('Gets order history data from "collected_orders" and "refunds" collections in MongoDB and renders items properly', async () => {
    ...
    expect(
      getAllByTestId("rendered-order-history-id-test")[0]
    ).toHaveTextContent("99");
    expect(getAllByTestId("order-history-item")[0]).toHaveTextContent(
      "Order Has Been Collected"
    );

    expect(
      getAllByTestId("rendered-order-history-id-test")[1]
    ).toHaveTextContent("98");
    expect(getAllByTestId("order-history-item")[1]).toHaveTextContent(
      "Refund Needed"
    );
  });
