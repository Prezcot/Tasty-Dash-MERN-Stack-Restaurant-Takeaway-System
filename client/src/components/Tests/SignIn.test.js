...
describe("UNIT TEST - SIGN IN COMPONENT", () => {
  ...
  it("User's Password Must Be Above 4 Characters", () => {
    ...
    expect(
      getByText("Please Enter A Password Above 4 Characters")
    ).toBeInTheDocument();
  });
});
...
describe("INTEGRATION TEST - SIGN IN COMPONENT", () => {
  it("User Is Able To Sign In", async () => {
    var { getAllByText, getByPlaceholderText } = render(
      <BrowserRouter>
        <SignIn />
      </BrowserRouter>
    );
    const expected_result = {
      data: { message: "Account Registered", user: "User" },
    };
...
describe("INTEGRATION TEST - SIGN IN COMPONENT", () => {
  it("User Is Taken To Menu Page After Signing In", async () => {
    var { getAllByText, queryByText, getByPlaceholderText } = render(
      <BrowserRouter>
        <SignIn />
      </BrowserRouter>
    );
