...
function Dashboard() {
  ...
  useEffect(() => {
    async function getUserInfo() {
      ...
      try {
        await axios
          .get(`http://localhost:3001/users/userinfo/${username}`, {
    ...
    if (current_password && new_password && confirm_password) {
      if (current_password.length >= 5 || new_password.length >= 5) {
        if (confirm_password == new_password) {
          try {
            await axios
              .put("http://localhost:3001/users/checkpassword", {
                username,
                current_password,
                new_password,
              })
  ...
  async function handleDelete() {
    const username = sessionStorage.getItem("username");
    await axios
      .delete(`http://localhost:3001/users/deleteaccount/${username}`)
      .then((res) => {
      ...
      .catch((err) => {
        sessionStorage.setItem("alert_msg", err.response.data.message);
      });
  }
  return (
    ...
    >
      ...
      >
        {!alert_msg ? (
          <div style={{ flex: 1 }}>
            <h1>Dashboard</h1>
            <p style={{ fontSize: "2vw", display: "inline" }}>
        ...
        <div>
          ...
          <form onSubmit={handleSubmit}>
            <table>
              <tr>
                <TabCol>
                  <p style={{ fontSize: "1.5vw", paddingTop: "2vh" }}>
                    Current Password:
                  </p>
                </TabCol>
                <TabCol>
